...
/*
 * Configure all peripherals used in this example.
*/
...
void get_power(void)
{
  ...
  uart_putstring((const uint8_t *)"Enter output power ('0'=+4 dBm, '1'=0 dBm,...,'7'=-40 dBm):");
  while(true)
  {
    c = uart_get();
    if ((c >= '0') && (c <= '7'))
...
void print_parameters(void)
{
  ...
  switch(txpower_)
  {
    ...
    case RADIO_TXPOWER_TXPOWER_Neg40dBm:
      uart_putstring((const uint8_t *)"TX Power............: -40 dBm\r\n");
      break;
  }
  ...
  while(true)
  {
    ...
    switch(uart_get())
    {
      case 'a':
        while(true)
        {
          uart_putstring((const uint8_t *)"Enter start channel (two decimal digits, 00 to 80):");
      ...
      case 'b':
        while(true)
        {
          uart_putstring((const uint8_t *)"Enter end channel (two decimal digits, 00 to 80):");
      ...
      case 'd':
        while(true)
        {
          uart_putstring((const uint8_t *)"Enter delay in ms (two decimal digits, 01 to 99):");
          delayms_ = get_dec2();
