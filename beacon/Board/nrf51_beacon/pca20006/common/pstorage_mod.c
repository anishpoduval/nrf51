 ...
 */
static __INLINE uint32_t cmd_queue_dequeue(void)
{
    uint32_t retval;
    retval = NRF_SUCCESS;
    ...
    if (m_cmd_queue.flash_access == true)
    {
        ...
        switch (sys_evt)
        {
            case NRF_EVT_FLASH_OPERATION_SUCCESS:
                ...
                m_round_val++;
                if ((m_round_val * SOC_MAX_WRITE_SIZE) >= p_cmd->size)
                {
                    app_notify(retval);
...
uint32_t pstorage_clear(pstorage_handle_t * p_dest, pstorage_size_t size)
{
    ...
    pages = m_app_table[p_dest->module_id].no_of_pages * PSTORAGE_FLASH_PAGE_SIZE;
