...
uint8_t acc_init(void) {
	...
	/* initialize GPIO */
	nrf_gpio_cfg_input(ACC_INT1, NRF_GPIO_PIN_NOPULL);
	nrf_gpio_cfg_input(ACC_MISO, NRF_GPIO_PIN_NOPULL);
	nrf_gpio_cfg_output(ACC_MOSI);
	nrf_gpio_cfg_output(ACC_SCK);
	nrf_gpio_cfg_output(ACC_nCS);

	/* mark inactive by default */
	nrf_gpio_pin_set(ACC_nCS);

	/* configure peripheral */
	SPI_ACC->PSELMISO = ACC_MISO;
	SPI_ACC->PSELMOSI = ACC_MOSI;
	SPI_ACC->PSELSCK = ACC_SCK;
